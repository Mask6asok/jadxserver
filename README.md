...
- **磁盘源码缓存** — 反编译后的 Java 源码保存到 `uploads/binary/<md5>/project.cache/code/sources/`，全量预热后搜索直接并行扫描磁盘缓存，避免反复撑大 JADX 堆
- **多格式支持** — 支持 APK、JAR、AAB、XAPK、APKS、APKM、DEX 等，通过 jadx 插件扩展

## 0.1.9 更新摘要

- **Manifest 结构化工具**：新增 `get_manifest_summary`、`list_manifest_permissions`、`list_manifest_components`、`search_manifest_components`、`list_manifest_intent_filters`、`get_manifest_entrypoints`，客户端可以直接查询精确字段，减少拉取完整 Manifest XML 后自行解析的上下文消耗。
- **搜索性能优化**：`decompile_apk` 会预热 JADX 磁盘源码缓存，`search_code` 和 `search_string` 优先并行扫描缓存源码，降低重复反编译导致的内存峰值。
- **8GB 运行配置**：默认 JVM 参数改为 `-Xmx8G`，低堆配置下自动收敛引擎实例数量；新增 `--engine-threads`，默认按 CPU 自动选择线程数，也可以显式提高 CPU 并行度。
- **缓存可观测性**：`decompile_apk` 返回 `code_cache` 状态、耗时和缓存文件数量，便于判断搜索是否已进入缓存路径。
...
| `get_manifest_summary` | 无 | 返回结构化 Manifest 摘要，包括包名、版本、SDK、platform build 版本及 application 关键属性 |
| `list_manifest_permissions` | 无 | 返回 Manifest 中的 `uses-permission`、声明的 `permission` 和 `uses-feature` 列表 |
| `list_manifest_components` | `type?` | 返回 activity、activity-alias、service、receiver、provider 组件及关键属性、intent-filter、meta-data；`type` 支持 `activity`、`activity_alias`、`service`、`receiver`、`provider`、`all` |
| `search_manifest_components` | `query`、`type?`、`regex?`、`case_sensitive?`、`limit?` | 使用 XML 解析搜索 Manifest 组件节点，命中后返回完整组件 XML 节点；`type` 过滤同 `list_manifest_components` |
| `list_manifest_intent_filters` | `type?` | 按组件展开 Manifest intent-filter，返回 action、category、data scheme/host/path/mimeType 等结构化字段；`type` 过滤同 `list_manifest_components` |
| `get_manifest_entrypoints` | 无 | 返回入口点视图，包括 launcher activity、exported 组件和 deeplink intent-filter |
...
输出：`build/libs/jadx-server-0.1.9-all.jar`（约 30MB）
...
```bash
java -Xms256M -Xmx8G -XX:+UseG1GC -jar build/libs/jadx-server-0.1.9-all.jar
java -Xms256M -Xmx8G -XX:+UseG1GC -jar build/libs/jadx-server-0.1.9-all.jar --xref-mode jadx
java -Xms256M -Xmx8G -XX:+UseG1GC -jar build/libs/jadx-server-0.1.9-all.jar --auth-token 'replace-with-a-strong-token'
```
...
```bash
java -jar jadx-server-0.1.9-all.jar --listen 127.0.0.1:8080
```

如需启用 HTTP 认证，启动时传入 token。未传入或传入空值时保持无需认证模式：

```bash
java -jar jadx-server-0.1.9-all.jar \
...
| `maxInstances` | `-m` / `--max-instances` | `0`（自动） | 最大引擎实例数；0 = 低堆内存自动 1，否则 `min(CPU/4, 2)` |
| `maxPerFile` | `--max-per-file` | `4` | 单个 APK/JAR 文件最大并发实例数 |
| `engineThreads` | `--engine-threads` | `0`（自动） | 单个 JADX 引擎的反编译/搜索线程数；0 = `min(CPU/2, 8)` |
